...
# first letter indicates whether the seat is in the front (0 through 63) 
...
def highest_seat(data)
    data.split(/\n+/)                                                   # RETURNS 998
    .map {|b_pass| b_pass.match(/(.{7})(.{3})/).captures}
...
def missing_seat(data)
    ...
    .minmax                                                             # GET LOWEST, HIGHEST VAL. RETURNS [38, 998]

    (38..998).each {|num| return num if !seats.include?(num)}           # RETURNS 676
end
